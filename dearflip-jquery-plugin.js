...
(function (root, factory) {
  ...
  Plugin.prototype = {
    ...
    _bindUI: function () {
      ...
      // Drag/Swipe para virar página
      var isDragging = false;
      var startX = 0;
      var currentPage = null;
      var originalTransform = '';

      this.$pages.on('mousedown touchstart', '.df-page', function(e){
        // Só permitir drag na página atual
        if(!$(this).hasClass('df-current')) return;
        
        isDragging = true;
        startX = e.type === 'touchstart' ? e.touches[0].clientX : e.clientX;
        currentPage = $(this);
        originalTransform = currentPage.attr('style') || '';
        e.preventDefault();
        e.stopPropagation();
      });
      ...
      $(document).on('mousemove touchmove', function(e){
        ...
        // calcula rotação proporcional ao arrasto
        var width = currentPage.width();
        var rotation = 0;
        
        if(currentPage.hasClass('df-left')) {
          rotation = Math.max(-180, Math.min(0, -delta / width * 180));
        } else if(currentPage.hasClass('df-right')) {
          rotation = Math.min(180, Math.max(0, delta / width * 180));
        }

        currentPage.css('transform', 'rotateY(' + rotation + 'deg)');
        e.preventDefault();
      });
      ...
      $(document).on('mouseup touchend', function(e){
        ...
        // virar página se arrasto passou de 50% da largura
        if(Math.abs(delta) > currentPage.width()/2){
          if(delta < 0) {
            self.next();
          } else {
            self.prev();
          ...
          currentPage.attr('style', originalTransform);
        }
